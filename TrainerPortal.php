...
<body>
  <div class="wrapper">
    ...
    if (isset($_POST['submit'])) {
      ...
      $result = mysqli_query($con, "SELECT * FROM trainer WHERE Email='$email' AND Password='$password' ") or die("Select Error");
      $row = mysqli_fetch_assoc($result);

      if (is_array($row) && !empty($row)) {
        $_SESSION['Trainervalid'] = $row['Email'];
        $_SESSION['username'] = $row['Name'];
        $_SESSION['password'] = $row['Password'];
        $_SESSION['contact'] = $row['Contact'];
        $_SESSION['id'] = $row['TrainerID'];
        $_SESSION['schdeuleid'] = $row['ScheduleID'];
        $_SESSION['salary'] = $row['Salary'];
      } else {
        echo "<div class='message'>
                 <p>Wrong Username or Password</p>
                  </div> <br>";
        echo "<a href='TrainerPortal.php'><button class='btn'>Go Back</button>";
      ...
      <form action="" method="post">
        ...
        <div class="register">
          <p>Don't have an account? <b><a href="TrainerRegister.php">Register</a></b></p>
        </div>
      </form>
    <?php } ?>
  </div>
